...
const formatPhoneNumber = (phone) => {
  ...
  // Add +62 prefix if number starts with 0 or 62
...
function SenderPage() {
  const [formData, setFormData] = useState({
    ...
    noRekening: '',
    jumlahTunggakan: '',
    macet: true,  // default true
    daftarHitam: true  // default true
  });
  ...
  const validateForm = (data) => {
    const errors = [];
    if (!data.namaNasabah?.trim()) errors.push('Nama Nasabah harus diisi');
    if (!data.nomorTelepon?.trim()) errors.push('Nomor Telepon harus diisi');
    if (!data.noRekening?.trim()) errors.push('Nomor Rekening harus diisi');
    if (!data.jumlahTunggakan) errors.push('Jumlah Tunggakan harus diisi');
    return errors;
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log('Form: Starting submission with data:', formData);
      
      // Format data for submission
      const preparedData = {
        nama_nasabah: formData.namaNasabah.trim(),
        nomor_telepon: formatPhoneNumber(formData.nomorTelepon),
        no_rekening: formData.noRekening.trim(),
        jumlah_tunggakan: parseFloat(formData.jumlahTunggakan.replace(/[,.]/g, '')),
        macet: Boolean(formData.macet),
        daftar_hitam: Boolean(formData.daftarHitam)
      };
      
      console.log('Form: Prepared data for submission:', preparedData);
      
      const result = await sendMessage(preparedData);
      ...
      setError(error.message || 'Terjadi kesalahan saat mengirim pesan');
      ...
      {notification && (
        ...
        {isWhatsAppReady && (
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid sm:grid-cols-2 gap-6">
              <div>
                <label htmlFor="macet" className={labelClassName}>Status Macet *</label>
                <select
                  id="macet"
                  value={formData.macet ? '1' : '0'}
                  onChange={(e) => setFormData({ ...formData, macet: e.target.value === '1' })}
                  className={inputClassName}
                  required
                >
              ...
              <div>
                <label htmlFor="daftarHitam" className={labelClassName}>Daftar Hitam *</label>
                <select
                  id="daftarHitam"
                  value={formData.daftarHitam ? '1' : '0'}
                  onChange={(e) => setFormData({ ...formData, daftarHitam: e.target.value === '1' })}
                  className={inputClassName}
                  required
                >
                  <option value="1">Ya</option>
